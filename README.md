...
- 使用完整字段路径（例如 `customer.amount`），支持多级关联，最多 4 段路径。
- 数值叶子字段支持合计、平均值、最大值、最小值；日期叶子字段支持极值。关联对象本身仅支持计数和非空计数。
- 「计数」表示主表行数；「非空计数」统计路径末端的非 null/undefined 值（空字符串沿用数据库 COUNT 语义，仍计数）。一对多、多对多路径展开关联值后计算；同一关联记录若出现在不同主表行中，按出现次数计入，不做全局去重。
- 全部数据模式保留当前主表筛选和关联区块上下文，并逐层校验目标表查看权限、字段权限与目标记录数据范围。
- 关联字段使用分批读取计算，单字段最多 10000 条主表记录、每层最多 50000 次关联记录出现；超限明确报错，不返回截断结果。大数据量请先缩小筛选范围。本表字段仍使用数据库聚合。
- 当前页模式仅计算页面已加载的关联值，不额外请求未加载的关联记录。
